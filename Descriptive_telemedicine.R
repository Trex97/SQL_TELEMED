...
#for the durg delivery 
#tele <- df %>%
#  filter(sub_fund %in% c("TELEMED","DRUG_DELIVERY"))
...
histogram <- ggplot(data = month_visit, mapping = aes(x = year_month,y = n ,fill = n)) +
  ...
  geom_text(aes(label = scales::comma(n)), vjust = -0.5, size = 5,fontface = "bold") +
  ...
  geom_rect(aes(xmin = ymd("2021-12-01"), xmax = ymd("2022-03-01"), ymin = 0, ymax = 25000),
            fill = NA, color = "orange", linetype = "dashed")+
  annotate(geom="text",x= ymd("2021-04-20"),col ="black",angle = 0, y=26000, label = "Alpha",size = 10)+
  annotate(geom="text",x= ymd("2021-07-7"),col ="black",angle = 0, y=26000, label = "Delta",size = 10)+
  annotate(geom="text",x= ymd("2022-01-20"),col ="black",angle = 0, y=26000, label = "Omicron",size = 10)+ 
  theme(
    plot.caption = element_text(),
    text = element_text(size = 25)  # ทำให้ข้อความทั้งหมดใน plot เป็นตัวหนา
  )
...
test_kmean <- tele_drug %>%
  select(pid,age,tran_id,age_group,sex,hcode,zone,NHSO_policy_des,sub_fund)

write_csv(test_kmean,"test_kmean.csv")
...
tele_drug <- tele_drug %>%
  mutate(mm = as.numeric(m)) 
tele_drug$mm <- sprintf("%02d",tele_drug$mm)

#change date from 1,2,3 to 01,02,03
tele_drug <- tele_drug %>%
  mutate(dd = as.numeric(d)) 
tele_drug$dd <- sprintf("%02d",tele_drug$dd)
...
##re type variable to date 
tele_drug$new_date <- dmy(tele_drug$new_date)
glimpse(tele_drug)
...
tele_drug <- tele_drug %>%
  mutate(year_month =format(tele_drug$new_date, "%Y-%m"))

#-----------------------------------end--------------------------------------

tele_ <- tele_drug %>%
  filter(sub_fund == "TELEMED")
tele_drug1 <- tele_drug %>%
  filter(sub_fund == "DRUG_DELIVERY")

tele_drug1 <- data.frame(unique(tele_drug1$tran_id))

tele_drug1$drug <- 1

colnames(tele_drug1) <- c("tran_id","drug")


tele_ <- tele_%>%
  mutate(tele = 1)

merged_table <- left_join(tele_, tele_drug1, by = "tran_id")


merged_table$drug  <- ifelse(is.na(merged_table$drug), 0, merged_table$drug)

merged_table <- merged_table %>%
  mutate(tele_drug = case_when(
    tele == 1 & drug == 1 ~ "With drug delivery",
    tele == 1 & drug == 0 ~ "Without drug delivery"
  ))

glimpse(merged_table)

table_drug_delivery <- merged_table %>%
  group_by(year_month,tele_drug,NHSO_policy_des) %>%
  count()

# Convert the year_month variable to a Date type
table_drug_delivery$year_month <- ym(table_drug_delivery$year_month)

glimpse(table_drug_delivery)

colnames(table_drug_delivery) <- c("year_month","type_of_delivery","NHSO_policy_des","n")
...
figure1 <- ggplot(data = table_drug_delivery, mapping = aes(x=year_month,y = n,fill =type_of_delivery )) + 
...
figure2 <- ggplot(data = table_drug_delivery, mapping = aes(x=year_month,y = n ,fill =type_of_delivery )) + 
  ...
  theme(axis.title.x = element_text(face = "bold")) +
  theme(axis.title.y = element_text(face = "bold"))+
  scale_fill_manual(values = c("Without drug delivery" = "red", "With drug delivery" = "green"))

figure1/figure2



#-------Facet each disease group ---------------
table_drug_delivery %>%
  group_by(NHSO_policy_des) %>%
  count()

#1.
drug_asthma <- table_drug_delivery %>%
  filter(NHSO_policy_des %in% "Asthma") %>%
  ggplot( aes(x=year_month, y=n, group=type_of_delivery,fill = type_of_delivery)) + 
  geom_bar(position="stack", stat="identity") +
  theme_minimal()+ 
  theme(axis.text = element_text(angle = 90, hjust = 1)) + 
  scale_x_date(date_labels = "%b-%Y", breaks = '1 month', expand = c(0.001, 5)) + 
  ...
  theme(
    plot.caption = element_text(),
    text = element_text(size = 22)  # ทำให้ข้อความทั้งหมดใน plot เป็นตัวหนา
  )+
  theme(axis.title.x = element_text(face = "bold")) +
  theme(axis.title.y = element_text(face = "bold")) + 
  theme(
...
#2. 
drug_cancers  <- table_drug_delivery %>%
  filter(NHSO_policy_des %in% "Cancers") %>%
  ggplot( aes(x=year_month, y=n, group=type_of_delivery,fill = type_of_delivery)) + 
  geom_bar(position="stack", stat="identity") +
  theme_minimal()+ 
  theme(axis.text = element_text(angle = 90, hjust = 1)) + 
...
#3.
drug_diabetes  <- table_drug_delivery %>%
  filter(NHSO_policy_des %in% "Diabetes") %>%
  ggplot( aes(x=year_month, y=n, group=type_of_delivery,fill = type_of_delivery)) + 
  geom_bar(position="stack", stat="identity") +
  theme_minimal()+ 
  theme(axis.text = element_text(angle = 90, hjust = 1)) + 
...
#4.
drug_hypertension  <- table_drug_delivery %>%
  filter(NHSO_policy_des %in% "Hypertension") %>%
  ggplot( aes(x=year_month, y=n, group=type_of_delivery,fill = type_of_delivery)) + 
  geom_bar(position="stack", stat="identity") +
  theme_minimal()+ 
  theme(axis.text = element_text(angle = 90, hjust = 1)) + 
...
#5.
drug_mental <- table_drug_delivery %>%
  filter(NHSO_policy_des %in% "Mental health") %>%
  ggplot( aes(x=year_month, y=n, group=type_of_delivery,fill = type_of_delivery)) + 
  geom_bar(position="stack", stat="identity") +
  theme_minimal()+ 
  theme(axis.text = element_text(angle = 90, hjust = 1)) + 
  ...
  labs(
    fill = "Types of service"
  )+
  scale_y_continuous(limits = c(0, 8000)) +
  scale_fill_manual(values = c("Without drug delivery" = "red", "With drug delivery" = "green"))

#6.
drug_other  <- table_drug_delivery %>%
  filter(NHSO_policy_des %in% "Other") %>%
  ggplot( aes(x=year_month, y=n, group=type_of_delivery,fill = type_of_delivery)) + 
  geom_bar(position="stack", stat="identity") +
  theme_minimal()+ 
  theme(axis.text = element_text(angle = 90, hjust = 1)) + 
...
drug_total<- (drug_asthma+drug_cancers+drug_diabetes)/(drug_hypertension+drug_mental+drug_other)
